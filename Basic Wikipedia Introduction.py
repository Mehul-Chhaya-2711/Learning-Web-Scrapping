...
def get_wikipedia_url(Search_Term):
    ...
    wikipedia_url = min(list(filter(lambda x: "wikipedia" in x and urllib.parse.quote(Search_Term).replace("%20","_") in x, URL3)),key = len)
...
def get_topic_introduction(wikipedia_scrape):
    ...
    my_tags = soup_object.find_all('p')[1].getText() #Get elements of second Paragraph tag
    print(my_tags)
    

get_topic_introduction(get_wikipedia_url("Bill Gates"))    
